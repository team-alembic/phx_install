defmodule Mix.Tasks.Phx.Install.Page do
  ...
  This task sets up:
  - `lib/<app>_web/controllers/page_controller.ex` — controller with `home/2` action
  - `lib/<app>_web/controllers/page_html.ex` — HTML view with `embed_templates`
  - `lib/<app>_web/controllers/page_html/home.html.heex` — stock homepage template
  - Browser scope with root route in the router

  ## Usage

      mix phx.install.page
  ...
  defp create_page_controller(igniter, web_module) do
  ...
  defp create_page_html(igniter, web_module) do
    page_html_module = Module.concat(web_module, PageHTML)
  ...
  defp has_browser_root_scope?(zipper, web_module) do
    case Igniter.Code.Function.move_to_function_call_in_current_scope(
           zipper,
           :scope,
           [2, 3],
           fn call_zipper ->
             Igniter.Code.Function.argument_equals?(call_zipper, 0, "/") &&
               scope_has_browser_pipe_through?(call_zipper) &&
               scope_matches_web_module?(call_zipper, web_module)
           end
         ) do
      {:ok, _} -> true
      :error -> false
  ...
  defp scope_has_browser_pipe_through?(scope_zipper) do
  ...
  defp scope_matches_web_module?(scope_zipper, web_module) do
    Igniter.Code.Function.argument_equals?(scope_zipper, 1, web_module)
  end

  defp add_browser_scope(zipper, code) do
    case Igniter.Code.Function.move_to_function_call_in_current_scope(
           zipper,
           :scope,
           [2, 3],
      ...
      {:ok, api_zipper} ->
        Igniter.Code.Common.add_code(api_zipper, code)
    ...
    <div class="px-4 py-10 sm:px-6 sm:py-28 lg:px-8 xl:px-28 xl:py-32">
      <div class="mx-auto max-w-xl lg:mx-0">
        ...
        <div class="mt-10 flex justify-between items-center">
          <h1 class="flex items-center text-sm font-semibold leading-6 text-zinc-900">
            Phoenix Framework
            <small class="ml-3 rounded-full bg-amber-100 px-2 py-0.5 text-xs font-medium text-amber-800">
        ...
        <div class="flex">
          <div class="w-full sm:w-auto">
            ...
            <div class="mt-10 grid grid-cols-1 gap-y-4 text-sm leading-6 text-zinc-500 sm:grid-cols-2">
              <div>
                <a
                  href="https://elixirforum.com"
                  class="group -mx-2 -my-0.5 inline-flex items-center gap-3 rounded-lg px-2 py-0.5 hover:bg-zinc-100 hover:text-zinc-900"
                >
                  <svg
                    viewBox="0 0 16 16"
                    aria-hidden="true"
